...
import type { RecurringInvoiceQueryParams } from '@/lib/types/finance';
import { RI_ITEMS_PER_PAGE, RI_STATUSES } from '@/lib/constants/recurring-invoice';

/** Owns the recurring-invoices list filters (search, status, page). */
export function useRecurringInvoiceFilters() {
  const [search, setSearch] = useState('');
  const [statusFilter, setStatusFilter] = useState('All');
  const [page, setPage] = useState(1);
  const [queryParams, setQueryParams] = useState<RecurringInvoiceQueryParams>({
    page: 1,
    limit: RI_ITEMS_PER_PAGE,
  });

  const build = useCallback((searchVal: string, status: string, pageNum: number) => {
    const params: RecurringInvoiceQueryParams = { page: pageNum, limit: RI_ITEMS_PER_PAGE };
    if (searchVal) params.search = searchVal;
    const entry = RI_STATUSES.find((s) => s.value === status);
