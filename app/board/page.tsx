...
function demoSessions(): BoardSession[] {
  ...
  return [
    ...
    s("a2", "Main Hall", "National Dialogue: Advancing Gender Equity in Academic Economics", -25, 20, "live", ["Prof. Chimezie Anyakora", "Ngozi Okonjo-Iweala"], "economic"),
    s("a3", "Main Hall", "Keynote: Financing the Care Economy", 35, 80, "scheduled", ["Amina J. Mohammed"], "economic"),
    s("a4", "Main Hall", "Closing Remarks and Communiqué", 95, 125, "scheduled", []),
    s("b1", "Hall B", "Health Forum: Maternal Outcomes at the Last Mile", -40, -5, "completed", ["Dr. Amara Okafor"], "health"),
    s("b2", "Hall B", "Digital Inclusion: Data Equity for Women-Led SMEs", 8, 53, "scheduled", ["Tunde Bakare", "Funke Opeke"], "digital"),
    s("b3", "Hall B", "GBV Forum: Survivor-Centred Justice", 65, 110, "scheduled", ["Ngozi Eze"], "gbv"),
    s("c1", "Pre-Function Area", "Innovation Hub Pitchathon: Round Two", -10, 50, "scheduled", ["Twelve finalists"], "innovation"),
...
function pickDay(sessions: BoardSession[], now: number): number | null {
  ...
  const upcoming = days.find((d) =>
    sessions.some((s) => s.day === d && +new Date(s.startsAt) > now),
  );
...
function buildLanes(sessions: BoardSession[], now: number, roomOrder: string[] | null): RoomLane[] {
  ...
  const rooms = roomOrder
    ...
    : [...byRoom.keys()].sort((a, b) => a.localeCompare(b));

  return rooms.map((room) => {
    const list = (byRoom.get(room) ?? []).sort(
    ...
    const byClock = list.find(
      (s) => s.status !== "completed" && +new Date(s.startsAt) <= now && now < +new Date(s.endsAt),
    );
    ...
    const after = list.filter(
      (s) =>
        s.status === "scheduled" &&
        s.id !== current?.id &&
        +new Date(s.startsAt) >= (current ? +new Date(current.endsAt) - 60_000 : now),
    );
  ...
  if (!s) {
    ...
    return (
      <motion.div key="idle" {...flip} className="flex h-full flex-col justify-center gap-[0.4vw]">
        <p className="text-[1.05vw] uppercase tracking-[0.2em] text-summit-smoke">
          {upcoming ? "Break" : "No further sessions"}
        </p>
        {upcoming && (
          <p className="text-[1.5vw] text-summit-lilac/70">
            Doors open {fmtTime(upcoming.startsAt)}
          </p>
        )}
      </motion.div>
    );
  ...
  return (
    <motion.div key={s.id} {...flip} className="flex h-full flex-col justify-center gap-[0.45vw]">
      <div className="flex items-center gap-[0.7vw]">
        {lane.isLive ? (
          <span className="inline-flex items-center gap-[0.5vw] rounded-full bg-summit-cerise/15 px-[0.9vw] py-[0.25vw] text-[0.95vw] font-semibold uppercase tracking-[0.18em] text-summit-cerise">
            <LiveDot /> Live
          </span>
        ) : (
          <span className="inline-flex items-center rounded-full bg-summit-lilac/10 px-[0.9vw] py-[0.25vw] text-[0.95vw] font-semibold uppercase tracking-[0.18em] text-summit-lilac/80">
            In progress
          </span>
        )}
        <span className="text-[1.05vw] tabular-nums text-summit-smoke">
          {fmtTime(s.startsAt)} – {fmtTime(s.endsAt)}
        </span>
        <span className="ml-auto flex items-baseline gap-[0.4vw] whitespace-nowrap pr-[0.4vw]">
          <span className="text-[0.9vw] uppercase tracking-[0.18em] text-summit-smoke">
            {overrun ? "Over by" : "Ends in"}
          </span>
          <Rolling
      ...
      <h2 className="line-clamp-2 font-[family-name:var(--font-archivo)] text-[1.85vw] font-bold leading-[1.1] tracking-[-0.02em]">
        {s.title}
      </h2>
      <Speakers session={s} />
    </motion.div>
...
function NextCell({ session, now, label }: { session: BoardSession | null; now: number; label: "Next" | "Then" }) {
  ...
  const startsIn = +new Date(session.startsAt) - now;
  const soon = startsIn <= 10 * 60_000;
  return (
    <motion.div key={session.id} {...flip} className="flex h-full flex-col justify-center gap-[0.45vw]">
      <div className="flex items-center gap-[0.7vw]">
        ...
        {label === "Next" && (
          <span className="ml-auto flex items-baseline gap-[0.4vw] whitespace-nowrap pr-[0.4vw]">
            <span className="text-[0.9vw] uppercase tracking-[0.18em] text-summit-smoke">In</span>
            <Rolling
              text={fmtRemaining(startsIn)}
              className={`font-[family-name:var(--font-archivo)] text-[1.5vw] font-bold leading-none tabular-nums ${soon ? "text-summit-cerulean" : "text-summit-lilac"}`}
            />
          </span>
        )}
      </div>
...
export default function BoardPage() {
  ...
  const day = useMemo(
    () => (sessions ? dayOverride ?? pickDay(sessions, now) : null),
    [sessions, now, dayOverride],
  );
  const lanes = useMemo(
  ...
  return (
    <main className="relative flex h-full w-full cursor-none flex-col overflow-hidden px-[3vw] py-[2.2vw]">
      ...
      <header className="relative flex items-end justify-between border-b border-summit-lilac/10 pb-[1.4vw]">
        <div>
          <p className="text-[1vw] uppercase tracking-[0.3em] text-summit-smoke">
            Gender &amp; Inclusion Summit 2026 · Abuja
          </p>
      ...
      */}
      <div
        ...
        style={{ zoom: Math.min(1, 3.4 / Math.max(1, lanes.length)) }}
      >
      {/* column legend */}
            ...
            {lanes.map((lane) => (
              ...
              >
                <div className="flex flex-col justify-center border-r border-summit-lilac/10 pr-[1.5vw]">
                  ...
                  <p className="mt-[0.2vw] font-[family-name:var(--font-archivo)] text-[1.9vw] font-bold leading-[1.05] tracking-[-0.02em]">
                    {lane.room}
                  </p>
                </div>
      ...
      <footer className="relative mt-[1.2vw] flex items-center justify-between border-t border-summit-lilac/10 pt-[1vw] text-[0.9vw] uppercase tracking-[0.25em] text-summit-smoke/70">
        ...
        <span className="flex items-center gap-[0.6vw]">
          <span
            className={`inline-block h-[0.6vw] w-[0.6vw] rounded-full ${stale ? "bg-summit-cream" : "bg-summit-green"}`}
          />
